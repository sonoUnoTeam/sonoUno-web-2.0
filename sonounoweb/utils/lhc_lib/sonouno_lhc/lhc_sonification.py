...
def save_individual_sound(sound, sleep_time, sound_path):
    """
    Create and save an individual sound file with the specified sleep time added as silence
    Parameters
    ----------
    sound : array, the sound array to save
    sleep_time : float, duration of silence to add after the sound
    sound_path : str, path where to save the sound file
    """
    # Create the complete sound with silence
    complete_sound = np.append(sound, get_silence(sleep_time))
    # Save the sound file
    wavfile.write(sound_path, rate=44100, data=complete_sound.astype(np.int16))


def reset_sound_to_save():
    """
    Reset the global sound_to_save variable to empty
    """
    global sound_to_save
    sound_to_save = np.array([])
